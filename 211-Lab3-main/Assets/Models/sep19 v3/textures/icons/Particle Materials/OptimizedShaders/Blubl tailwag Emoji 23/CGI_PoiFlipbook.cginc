...
half2 _AudioLinkFlipbookFrame;
    ...
    void applyFlipbook(inout float4 finalColor, inout float3 flipbookEmission)
    {
        ...
        if (float(0) == 0)
        {
        ...
        #if defined(PROP_FLIPBOOKTEXARRAY) || !defined(OPTIMIZER_ENABLED)
            float currentFrame = fmod(float(-1), float(10));
            if (float(-1) < 0)
            {
                currentFrame = (_Time.y / (1 / float(10))) % float(10);
            }
            #ifdef POI_AUDIOLINK
            ...
            if (float(0))
            {
                float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(10))));
