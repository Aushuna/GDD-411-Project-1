    ...
    bool shouldMetalHappenBeforeLighting()
    {
        ...
        return probeExists && !(1.0 /*_SampleWorld*/);
    }
    ...
    {
        metalicMap = POI2D_SAMPLER_PAN(_MetallicMask, _MainTex, poiMesh.uv[(0.0 /*_MetallicMaskUV*/)], float4(0,0,0,0)) * (1.0 /*_Metallic*/);
    }
    void ApplyMetallics(inout float4 finalColor, in float4 albedo)
    {
        #ifdef FORWARD_BASE_PASS
            float smoothnessMap = (POI2D_SAMPLER_PAN(_SmoothnessMask, _MainTex, poiMesh.uv[(0.0 /*_SmoothnessMaskUV*/)], float4(0,0,0,0)));
            #ifdef POI_BLACKLIGHT
                if ((4.0 /*_BlackLightMaskMetallic*/) != 4)
                {
                    metalicMap *= blackLightMask[(4.0 /*_BlackLightMaskMetallic*/)];
                    smoothnessMap *= blackLightMask[(4.0 /*_BlackLightMaskMetallic*/)];
                }
            #endif
            if((0.0 /*_InvertSmoothness*/) == 1)
            {
                smoothnessMap = 1 - smoothnessMap;
            }
            smoothnessMap *= (1.0 /*_Smoothness*/);
            ...
            half4 tintMap = POI2D_SAMPLER_PAN(_MetallicTintMap, _MainTex, poiMesh.uv[(0.0 /*_MetallicTintMapUV*/)], float4(0,0,0,0));
            finalColor.rgb *= (1 - metalicMap * tintMap.a);
            finalColor.rgb += reflecty_lighty_boy_uwu_var_2 * reflection.rgb * fresnelRelflection(albedo) * float4(1,0.7069076,0.3349057,1) * tintMap.rgb * tintMap.a;
        #endif
    }
