...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(1,1,1,0.6235294).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, (0.0 /*_MainVertexColoring*/)), mainTexture.a * max(float4(1,1,1,0.6235294).a, 0.0000001));
        ...
        float mixedHueShift = (0.987 /*_MainHueShift*/);
